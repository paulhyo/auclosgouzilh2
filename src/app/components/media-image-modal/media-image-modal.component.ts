...
@Component({
  selector: 'media-image-modal',
  template: `
   <div class="media-left media-middle" *ngFor ="let i of modalImages; let index = index">
     <img src="{{ i.thumb }}" class="media-object img-responsive img-thumbnail ng-thumb" (click)="openGallery(index)" alt="Image {{ index + 1 }}" />
   </div>
   <div class="ng-overlay" *ngIf="opened">
    ...
    <a class="close-popup" (click)="closeGallery()"><i class="fa fa-close"></i></a>
     ...
     <span class="info-text">{{ currentImageIndex + 1 }}/{{ modalImages.length }} - Image {{currentImageIndex+1}}</span>
       ...
       `,
  styleUrls: ['./media-image-modal.component.css']
})
export class MediaImageModalComponent implements OnInit {
  public _element:any;
